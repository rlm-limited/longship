...
from longship_api_client.api.chargepoint_status import chargepoint_status_get, get_all_chargepointstatus
from longship_api_client.api.chargepoints import chargepoint_get, get_all_chargepointmessages
from longship_api_client.api.commands import send_get_composite_schedule_request, send_set_charging_profile_request
from longship_api_client.api.sessions import get_all_sessions
from longship_api_client.models import GetCompositeScheduleRequest, SetChargingProfileRequest
...
from longship_api_client.models.charging_schedule_charging_rate_unit import ChargingScheduleChargingRateUnit
...
from longship_api_client.models.cs_charging_profiles_charging_profile_kind import CsChargingProfilesChargingProfileKind
...
class Longship:
    ...
    def __init__(self, url: str, apiKey: str, ocpKey: str) -> None:
        self._client = Client(base_url=url).with_headers({"x-api-key": apiKey, "Ocp-Apim-Subscription-Key": ocpKey})

    async def get_composite_schedule(
        self, chargepoint_id: str, connector_id=0, duration=3600, unit=GetCompositeScheduleRequestChargingRateUnit.W
    ) -> ChargingSchedule:
        request = GetCompositeScheduleRequest(connector_id=connector_id, duration=duration, charging_rate_unit=unit)
        ...
        response = await self.get_messages(chargepoint_id, response_only=True, message_id=message_id)
    ...
    def _create_charging_schedule(self, max_power: int, min_power: int = 1000, number_of_phases: int = 3):
        period = ChargingSchedulePeriod(start_period=0, limit=max_power, number_phases=number_of_phases)
    ...
    async def set_charge_point_max_power(
        self, chargepoint_id: str, max_power: int, min_power: int = 1000, number_of_phases: int = 3
    ):
        schedule = self._create_charging_schedule(max_power, min_power, number_of_phases)
        ...
        request = SetChargingProfileRequest(connector_id=0, cs_charging_profiles=profiles)
    ...
    async def set_transaction_max_power(
        ...
        request = SetChargingProfileRequest(connector_id=connector_id, cs_charging_profiles=profiles)
        return await send_set_charging_profile_request.asyncio_detailed(
            id=chargepoint_id, json_body=request, client=self._client
        )

    async def get_messages(
        self, chargepoint_id: str, response_only=False, message_id=None, message_types=None
    ) -> List[MessageLogDto]:
    ...
    async def get_chargepoint(self, chargepoint_id: str) -> ChargepointDto:
        response = await chargepoint_get.asyncio_detailed(id=chargepoint_id, client=self._client)
        return response.parsed

    async def list_chargepoint_statuses(self, skip: int = None, take: int = None) -> ChargepointStatusDto:
        response = await get_all_chargepointstatus.asyncio_detailed(skip=skip, take=take, client=self._client)
        return response.parsed

    async def get_chargepoint_status(self, chargepoint_id: str) -> ChargepointStatusDto:
        response = await chargepoint_status_get.asyncio_detailed(id=chargepoint_id, client=self._client)
        return response.parsed

    async def get_sessions(self, chargepoint_id=None, connector_number=None, running_only=True) -> List[SessionDto]:
